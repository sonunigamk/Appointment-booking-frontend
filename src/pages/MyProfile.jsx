...
const MyProfile = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    address: {
      ...
      line2: "cricle, Church Road, London"
    },
    geneder: 'Male',
    dob: '2000-01-20'
  })
            ...
            isEdit
              ? <p>
                <input className='bg-gray-50'
                 onChange={(e) => setUserData(prev => ({ ...prev, address, line1: e.target.value }))} value={userData.address.line1} type="text" />
                <br />
                <input className='bg-gray-50'
                 onChange={(e) => setUserData(prev => ({ ...prev, address, line2: e.target.value }))} value={userData.address.line2}
                  type="text" />
              </p>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          <p className='font-medium'>Gender:</p>
          {
            isEdit
              ...
              onChange={(e) => setUserData(prev => ({ ...prev, geneder: e.target.value }))} value={userData.geneder}>
                <option value="Male">Male</option>
                <option value="Female">Female</option>

              </select>
              : <p className='text-gray-400'>{userData.geneder}</p>
          }
